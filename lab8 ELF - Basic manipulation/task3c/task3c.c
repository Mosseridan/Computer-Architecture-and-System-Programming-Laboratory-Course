...
void print_sym_tab(Elf_Info* elfi ,Elf32_Word sh_type){
  ...
  printf("\nSymbol table '%s' contains %d entries:\n",get_sh_name(get_sym_tab_ndx(elfi, sh_type), elfi),sym_num);
...
/* lookup a given symbol in the given elf info list.
  returns the index of the given symbol in the given elf infos symbol tab or 0 if not found */
  ...
  while(elfi != NULL){
    for(i = 0; i < elfi->sym_num; i++){
      if(strcmp(elfi->str_tab + elfi->sym_tab[i].st_name, symbol) == 0
          && elfi->sym_tab[i].st_shndx != SHN_UNDEF){
        return i;
      }
    }
    elfi = elfi->next;
...
/* determines whether there are duplicate symbols defined in on more then one file */
void duplicate_symbol_check(Elf_Info* elfi){
  int i, dup_count = 0;
  while(elfi != NULL){
    for(i = 0; i < elfi->sym_num; i++){
      if(elfi->sym_tab[i].st_shndx != SHN_UNDEF
        && (strcmp(sym_name, "") != 0)
        && lookup_symbol(elfi->next, elfi->str_tab + elfi->sym_tab[i].st_name)){
           printf("\tduplicate symbol: %s\n", elfi->str_tab + elfi->sym_tab[i].st_name);
           dup_count++;
      }
    }
    elfi = elfi->next;
  }
  if(dup_count == 0)  printf("\tduplicate check: PASSED\n");
}

/* determines whether there aresymbols that are undefined in the files */
void undefined_check(Elf_Info* elfi){
  int i, und_count = 0;
  Elf_Info* first = elfi;
  char* sym_name;
  while(elfi != NULL){
    for(i = 0; i < elfi->sym_num; i++){
      sym_name = elfi->str_tab + elfi->sym_tab[i].st_name;
      if(elfi->sym_tab[i].st_shndx == SHN_UNDEF
          && (strcmp(sym_name, "") != 0)
          && (lookup_symbol(first, elfi->str_tab + elfi->sym_tab[i].st_name) == 0)){
  ...
  if(und_count == 0)  printf("\tno undefined symbols: PASSED\n");
}
...
void link_to(){
  ...
  if(ELFI->next == NULL){
  	...
  	clear_elf_info(ELFI->next);
  }
  if(open_and_map_elf(ELFI->next)){
    printf("\nTesting files:\n");
    start_check(ELFI);
    duplicate_symbol_check(ELFI);
    undefined_check(ELFI);
  }
}
