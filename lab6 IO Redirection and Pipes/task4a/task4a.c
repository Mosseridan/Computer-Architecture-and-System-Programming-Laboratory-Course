...
/* redirects proccess input, if needed */
 void redirect_input(cmdLine *pCmdLine){
...
/* redirects proccess output, if needed */
 void redirect_output(cmdLine *pCmdLine){
...
/*This function receives an array of pipes and a pointer to a cmdLine structure.
 ...
 void execute(cmdLine *pCmdLine) {
   int res;
   redirect_input(pCmdLine);
   redirect_output(pCmdLine);
 ...
 void test_pipes(){
    char* entry = "ls | tee | cat | more";
    cmdLine* pCmdLine =  parseCmdLines(entry);
    int** pipes = createPipes(3);

    printf("TESTING PIPES IMPLEMENTATION:\n");
    cmdLine* temp = pCmdLine;
    if(leftPipe(pipes,temp) == NULL){
      printf("PASS: ls left pipe is NULL\n");
    }
    else{
      printf("FAIL: ls left pipe is %d\n",leftPipe(pipes,temp)[1]);
    }
    printf("ls right pipe fd is: %d\n", rightPipe(pipes,temp)[0]);
    temp = pCmdLine->next;
    printf("tee left pipe fd is: %d\n", leftPipe(pipes,temp)[1]);
    printf("tee right pipe fd is: %d\n", rightPipe(pipes,temp)[0]);
    temp = temp->next;
    printf("cat left pipe fd is: %d\n", leftPipe(pipes,temp)[1]);
    printf("cat right pipe fd is: %d\n", rightPipe(pipes,temp)[0]);
    temp = temp->next;
    printf("more left pipe fd is: %d\n", leftPipe(pipes,temp)[1]);
    if(rightPipe(pipes,temp) == NULL){
      printf("PASS: more right pipe is NULL\n");
    }
    else{
      printf("FAIL: more right pipe is %d\n",rightPipe(pipes,temp)[0]);
    }
    freeCmdLines(pCmdLine);
