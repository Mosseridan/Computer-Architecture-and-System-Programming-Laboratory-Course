...
void duplicate_symbol_check(Elf_Info* elfi){
  int i,ndx;
  Elf_Info* other; = elfi->next;
  while(elfi != NULL){
    other = elfi->next;
    while(other != NULL){
      for(i = 0; i < elfi->sym_num; i++){
        if(elfi->sym_tab[i].st_shndx != SHN_UNDEF){
          ndx = lookup_symbol(other, elfi->str_tab + elfi->sym_tab[i].st_name);
          if(ndx != 0 && other->sym_tab[ndx].st_shndx != SHN_UNDEF){
            printf("\tduplicate symbol: %s", elfi->str_tab + elfi->sym_tab[i].st_name);
            return;
          }
        }
      }
