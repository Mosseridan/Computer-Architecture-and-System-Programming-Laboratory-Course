...
/* replaces a unit at address in hexeditplus memory by val*/
void mem_modify(){
  char* address;
  char* input;
  int val, i;
  ...
  input = get_input();
  if(input != NULL && sscanf(input,"%p %X",&address,&val) == 2){
    if(address == 0){
      address = BUFF;
    }
    dprint(stderr,"\nDebug:\tfile name: %s\n\taddress: %p\n\tval: %02X\n\n",FILE_NAME,address,val);
    for(i = 0; i<UNIT_SIZE;i++){
      address[i] = ((unsigned char*)&val)[i];
    }
  }
  else{
    fprintf(stderr,"Error: expected <s-address> <t-location> <length> but given %s",input);
...
int main(int argc, char **argv){
  ...
  fun_desc fdarray[] = {
    ...
    create_fun_desc("Mem Modify",mem_modify),
