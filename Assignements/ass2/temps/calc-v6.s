...
b1:
  ...
  jnz %%.not_debug
  inc ecx
  is_new_line? byte [BUFFER+ecx], %%.not_debug
  jnz %%.not_debug
  mov dword [DEBUGE], 1
  %%.not_debug:
  mov dword [DEBUGE], 0
...
%macro is_operand? 0
  mov ecx, 0
  %%.operand_loop:
  is_new_line? byte [BUFFER+ecx], %%.operand_code
