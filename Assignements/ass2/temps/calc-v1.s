...
%macro input_check 1                                          ; check if BUFFER[0] is operator. operator- pop operands
                                                              ; operand- ret 1
  cmp %1, '+'
  jz pop_tow_operands
  cmp %1, '&'
...
my_calc:
  ...
  my_calc_code:
    ...
    input_check                 ; check the input- operator/ operand
    cmp eax, 1                  ; if input is not an operand- jump to end
    jnz .end
    push_op                     ; else push input(operand) into OP_STACK
    .end:
    is_end?
