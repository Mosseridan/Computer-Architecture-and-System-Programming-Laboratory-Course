...
init_func
  mov ecx, eax
  mov eax, [ebp+8]  ;get cor
  mov edx, [ebp+12] ; get row
...
cell_func:
  ...
  sub eax, 1 ; get cell
  mov ebx, [state+eax] ; get current cell content
  mov dword[cur], ebx ; save current cell content
  mov dword[current_cell], eax ; save current cell
  mov edx, eax
  .neighbors:
  mov ebx, edx
  add ebx, 1 ; left neighbor
  push ebx
  call check_neighbors
  mov ebx, edx ; right neighbor
  sub ebx, 1
  call check_neighbors
  mov ebx, dword[x] ; up and right neighbor
  sub ebx, 1
  mov ecx, dword[y]
  sub ecx, 1
  push ecx
  push ebx
  call get_neighbor
  mov ebx, dword[x] ; up and left neighbor
  sub ebx, 1
  mov ecx, dword[y]
  add ecx, 1
  push ecx
  push ebx
  call get_neighbor
  mov ebx, dword[x] ; down and right neighbor
  add ebx, 1
  mov ecx, dword[y]
  sub ecx, 1
  push ecx
  push ebx
  call get_neighbor
  mov ebx, dword[x] ; down and left neighbor
  add ebx, 1
  mov ecx, dword[y]
  add ecx, 1
  push ecx
